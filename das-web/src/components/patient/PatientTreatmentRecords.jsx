            ...
            <select value={selectedRecord?._id || ""} onChange={(event) => chooseRecord(event.target.value)}>
              {records.map((record) => (
                <option key={record._id} value={record._id}>
                  {record.appointment?.service?.name || "Dịch vụ"} - {formatDateTime(record.appointment?.startAt || record.treatmentDate)}
                </option>
              ))}
            </select>
          </label>
            ...
            <>
              ...
              <div className="readonly-record-grid">
                <div className="clinical-selected-card wide">
                  <strong>{selectedRecord.appointment?.service?.name || "Hồ sơ điều trị"}</strong>
                  <span>Bác sĩ: {selectedRecord.dentist?.fullName || "-"}</span>
