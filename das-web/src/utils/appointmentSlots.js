...
export const exactTimeStatuses = new Set(["confirmed", "checked_in", "in_treatment", "completed"]);
...
export function minutesOfClinicDay(value) {
  const parts = clinicDateTimeParts(value);
  if (!parts) return 0;
...
export function formatSlotOnly(value) {
...
export function shouldDisplayExactTime(status) {
  return exactTimeStatuses.has(status);
}
